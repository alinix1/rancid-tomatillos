...
class SingleMovie extends Component {
  ...
  componentDidMount = () => {
    ...
    fetchAllData(`/movies/${this.props.id}/videos`)
      ...
      .catch((error) =>
        this.setState({
          ...this.state,
          videoErrorMessage: "Sorry, error displaying videos."
        })
      );
  };
  ...
  render() {
    ...
    const { backdrop_path, title, overview, release_date, genres, runtime } =
      this.state.movie;
    return (
      <section className="single-movie-container">
        ...
        <section className="movie-detail-wrapper">
          <div className="button">
            <Link to="/">
              <img src={close} alt="close" className="close-symbol" />
            </Link>
          </div>
          <h2>{title}</h2>
